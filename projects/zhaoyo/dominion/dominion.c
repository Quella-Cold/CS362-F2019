...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    case mine:
       return (card_mine(&j, state, currentPlayer, choice1, choice2, handPos));
    ...
    case baron:
        card_baron(choice1,state,currentPlayer,nextPlayer);
        return 0;
    ...
    case minion:
        card_minion(state,choice1,choice2,handPos,currentPlayer);
        return 0;
    ...
    case tribute:
        card_tribute(state,nextPlayer,tributeRevealedCards,currentPlayer);
        return 0;

    case ambassador:
        return  (card_ambassador(choice2,choice1,handPos,state,currentPlayer));
...
int card_mine(int *j, struct gameState *state, int currentPlayer, int choice1, int choice2, int handPos){
        ...
        *j = state->hand[currentPlayer][choice1];  //store card we will trash
        ...
        if (state->hand[currentPlayer][choice1] < copper && state->hand[currentPlayer][choice1] > gold) //the player can trash any kinds of card
        {
            return -1;
        }
        ...
        //discard trashed card
        for (i = 0; i < (state->handCount[currentPlayer])-1; i++)   //if the player choose the last card, the card will not be dropped.
        {
            if (state->hand[currentPlayer][i] == *j)
            {
                discardCard(i, currentPlayer, state, 0);
...
void card_baron(int choice1,struct gameState *state,int currentPlayer,int nextPlayer){
        ...
        if (choice1 > 0) { //Boolean true or going to discard an estate
            ...
            while(card_not_discarded) {
                if (state->hand[currentPlayer][p] == estate) { //Found an estate card!
                    ...
                    if (supplyCount(estate, state) > 0) {
                        ...
                        state->supplyCount[estate]--;//Decrement estates  //The back end will not update the number of estate card
                        if (supplyCount(estate, state) == 0) {
                            isGameOver(state);
                        }
                    }
            ...
            if (supplyCount(estate, state) > 0) {
                gainCard(estate, state, 0, nextPlayer);//Gain an estate //transfer the estate card from current player to the next player
...
void card_minion(struct gameState *state,int choice1,int choice2,int handPos,int currentPlayer){
    int i,j;
            //+1 action
        state->numActions++;
        ...
        else if (choice2)        //discard hand, redraw 4, other players with 5+ cards discard hand and draw 4
        {
            ...
            for (i = 0; i < state->numPlayers; i++)
            {
                if (i != currentPlayer)
                {
                    if ( state->handCount[i] >= 4 ) //while others player have 4 or more than 4 number of hand cards, shuffle their hand card!!!!
                    {
                        //discard hand
                        while( state->handCount[i] > 1 )    //leave one hand card for these player
                        {
...
int card_ambassador(int choice2,int choice1,int handPos, struct gameState *state,int currentPlayer){
    int i,j;
    ...
    for (i = 0; i < (state->numPlayers)-1; i++) //the last player cannot gain a copy of revealed card!!!!!!!!!!!!!!!!!!!!!!!!!!
    {
        if (i != currentPlayer)
        {
